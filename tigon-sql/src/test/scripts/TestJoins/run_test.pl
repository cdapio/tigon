...
$JoinCount = 0;
foreach my $JoinType (@Joins)
{
    if ( $JoinCount++ > 0 )
    { LogMessage "===============================================================", NO_TIMESTAMP; }
    my $JoinQueryName = undef;
    chdir "$ROOT/$JoinType" or Die "Could not change directory to $ROOT/$JoinType: $!";
####    LogMessage "Working on join $JoinType in $ROOT/$JoinType.";
    LogMessage "Starting: <$TestName/$JoinType>", NO_TIMESTAMP;
    ...
    foreach my $QueryFile ( @Queries )
    {
        ...
        if ( $QueryFile eq "JoinQuery.gsql" )
        {
            $JoinQueryName = ${QueryName};
        }
    }
    close OUTSPEC;

    Die "Could not locate the expected query files in $ROOT/$JoinType."
    if ( ! defined $JoinQueryName or @Queries < 3 );
    ...
    foreach my $FileToDelete qw( Output.txt envy4.research.att.com_lfta.c gswatch.pl hfta_0 hfta_0.cc hfta_0.o lfta.o Makefile postopt_hfta_info.txt preopt_hfta_inf qtree.xml rts runit set_vinterface_hash.bat stopit  internal_fcn.def external_fcns.def  )
    {
        foreach my $File ( @DirectoryList )
        {
    ...
    {
        LogMessage "buildit for $JoinType failed.", SYSERROR;
        LogMessage "Ending: <$TestName/$JoinType> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
        next;
    }
    LogMessage "buildit in $ROOT/$JoinType successful.";
    KillAll;
...
#    my $RC = system("./runit > run.out 2>&1");
    ...
    if ( length($IP) < 10 )
    {
        LogMessage "Can not find hub ip in $ROOT/$JoinType.", SYSERROR;
        LogMessage "Ending: <$TestName/$JoinType> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
        next;
    }

    print "${STREAMING}/bin/gsgdatprint $IP default $JoinQueryName\n";
    system("${STREAMING}/bin/gsgdatprint $IP default $JoinQueryName > gsgdatprint.out 2>&1&");
...
#    Die "Could not run gen_feed." unless Ps("gen_feed") == 1;
    ...
    if ( @DataFiles == 0 )
    {
        LogMessage "Can not locate text files expected from gsgdatprint for $JoinType.", FAILURE;
        LogMessage "Ending: <$TestName/$JoinType> <$ErrorStrings{FAILURE}>", NO_TIMESTAMP;
        next;
    }
    ...
    if ( ! -f "Expected.txt" )
    {
        LogMessage "Could not locate file Expected.txt in $ROOT/$JoinType.", SYSERROR;
        LogMessage "Ending: <$TestName/$JoinType> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
        next;
    }
    open FILE, "Expected.txt" or Die "Could not open Expected.txt for read.";
    ...
    {
        LogMessage "Could not retrieve expected lines from file Expected.txt in $ROOT/$JoinType.", SYSERROR;
        LogMessage "Ending: <$TestName/$JoinType> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
        next;
    }
    ...
    foreach my $File (@DataFiles)
    {
        ...
        {
            ++$Exceptions;
            LogMessage "Exceptions found for type $JoinType.", FAILURE;
        ...
        {
            LogMessage "No differences found for $JoinType.";
            LogMessage "Ending: <$TestName/$JoinType> <$ErrorStrings{SUCCESS}>", NO_TIMESTAMP;
        }
    ...
    if ( $DiffChecked == 0 )
    {
        LogMessage "Can not identify valid text file expected from gsgdatprint for join $JoinType.", FAILURE;
    }
}
