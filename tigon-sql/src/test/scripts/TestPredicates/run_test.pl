...
$P_Count = 0;
foreach my $P_Type (@Predicates)
{
    if ( $P_Count++ > 0 )
    { LogMessage "===============================================================", NO_TIMESTAMP; }
    chdir "$ROOT/$P_Type" or Die "Could not change directory to $ROOT/$P_Type: $!";
####    LogMessage "Working on predicate $P_Type in $ROOT/$P_Type.";
    LogMessage "Starting: <$TestName/$P_Type>", NO_TIMESTAMP;
...
###            push @QueryFiles, $Item;
            ...
            {
                LogMessage "Could not extract query name from $Item in $ROOT/$P_Type.", SYSERROR;
                next;
            }
    ...
    Die "Could not locate any query files in $ROOT/$P_Type." if ( ! @QueryNames );
...
sleep 2;
    ...
    close FILE;
    Die "Could not chmod on $ROOT/$P_Type/gen_feed." unless chmod(0777, "gen_feed") == 1;
    ...
    {
        LogMessage "buildit for $P_Type failed.", SYSERROR;
        LogMessage "Ending: <$TestName/$P_Type> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
        next;
    }
    LogMessage "buildit in $ROOT/$P_Type successful.";
    foreach my $QueryName ( @QueryNames )
    {
        LogMessage "Starting: <$TestName/$P_Type/$QueryName>", NO_TIMESTAMP;
        opendir DIR, "." or Die "Could not open current directory for read.";
        @DirectoryList = readdir DIR;
        closedir DIR;
        foreach my $File ( @DirectoryList )
...
#        my $RC = system("./runit > run.out 2>&1");
    ...
    # CHILD
        ...
        if ( $RC != 0  )
        {
            ...
            LogMessage "Ending: <$TestName/$P_Type/$QueryName> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
            next;
        }
        ...
        if ( length($IP) < 10 )
        {
            LogMessage "Can not find hub ip in $ROOT/$P_Type.", SYSERROR;
            LogMessage "Ending: <$TestName/$P_Type/$QueryName> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
            next;
        }
   ...
   #     Die "Could not run gen_feed." unless Ps("gen_feed") == 1;
        ...
        {
            LogMessage "Could not locate file Expected_${QueryName}.txt in $ROOT/$P_Type.", SYSERROR;
            LogMessage "Ending: <$TestName/$P_Type/$QueryName> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
            next;
        }
        ...
        {
            LogMessage "Could not retrieve expected lines from file Expected_${QueryName}.txt in $ROOT/$P_Type.", SYSERROR;
            LogMessage "Ending: <$TestName/$P_Type/$QueryName> <$ErrorStrings{SYSERROR}>", NO_TIMESTAMP;
            next;
        }
        ...
        {
            sleep 2;
            InspectOutput($TestName, $P_Type, $QueryName, $Count, $ExpectedLineCount, $ExpectedContent, $DiffChecked);
        }

        if ( $DiffChecked == 0 )
        {
            LogMessage "Can not identify valid text file expected from gsgdatprint for predicate $P_Type/$QueryName.", FAILURE;
            LogMessage "Ending: <$TestName/$P_Type/$QueryName> <$ErrorStrings{FAILURE}>", NO_TIMESTAMP;
        }
    }
}
...
sub InspectOutput($$$$$$\$)
{
    my ($TestName, $P_Type, $QueryName, $NextLoop, $ExpectedLineCount, $ExpectedContent, $DiffChecked) = @_;
    LogMessage "Checking gsgdatprint output for $P_Type . . .";
    ...
    if ( @DataFiles == 0 )
    {
        LogMessage "Can not locate text files expected from gsgdatprint for $TestName/$P_Type/$QueryName.", FAILURE;
        LogMessage "Ending: <$TestName/$P_Type/$QueryName> <$ErrorStrings{FAILURE}>", NO_TIMESTAMP;
        next;
    }
    ...
    foreach my $File (@DataFiles)
    {
        ...
        {
            LogMessage "Exceptions found for type $P_Type.", FAILURE;
        ...
        {
            LogMessage "No differences found for $TestName/$P_Type/$QueryName.";
            LogMessage "Ending: <$TestName/$P_Type/$QueryName> <$ErrorStrings{SUCCESS}>", NO_TIMESTAMP;
        }
